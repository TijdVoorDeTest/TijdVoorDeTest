...
use Tvdt\Controller\Molshoop\PrepareEliminationController;
...
final class PrepareEliminationControllerTest extends AbstractControllerWebTestCase
{
    ...
    public function testIndexCreatesEliminationAndRedirectsToView(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage(\sprintf('/molshoop/season/krtek/quiz/%s/result', $quiz->id), '/elimination/prepare');

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/quiz/%s/elimination/prepare', $quiz->id), [
            '_token' => $token,
        ]);

        $response = $this->client->getResponse();
        $this->assertTrue($response->isRedirect());
        $this->assertStringContainsString('/molshoop/elimination/', (string) $response->headers->get('Location'));
    ...
    public function testViewEliminationPageLoads(): void
    {
        ...
        $this->client->request(Request::METHOD_GET, \sprintf('/molshoop/elimination/%s', $elimination->id));
    ...
    public function testViewEliminationShowsCandidatesOrderedByScoreWithTime(): void
    {
        ...
        $crawler = $this->client->request(Request::METHOD_GET, \sprintf('/molshoop/elimination/%s', $elimination->id));
    ...
    public function testViewEliminationSavesUpdatedColours(): void
    {
        ...
        $token = $this->getTokenFromPage(\sprintf('/molshoop/elimination/%s', $elimination->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/elimination/%s', $elimination->id), [
            '_token' => $token,
            'colour-tom' => ScreenColour::Red->value,
            'start' => '0',
        ]);

        self::assertResponseRedirects(\sprintf('/molshoop/elimination/%s', $elimination->id));
        $this->entityManager->clear();
    ...
    public function testDeleteEliminationRemovesEliminationAndRedirectsToQuiz(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage(\sprintf('/molshoop/elimination/%s', $elimination->id), \sprintf('/molshoop/elimination/%s/delete', $elimination->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/elimination/%s/delete', $elimination->id), [
            '_token' => $token,
        ]);

        self::assertResponseRedirects(\sprintf('/molshoop/season/krtek/quiz/%s', $quiz->id));
        $this->entityManager->clear();
