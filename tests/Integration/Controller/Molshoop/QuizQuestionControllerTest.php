...
use Tvdt\Controller\Molshoop\QuizQuestionController;
...
final class QuizQuestionControllerTest extends AbstractControllerWebTestCase
{
    public function testEditPreservesAnswerOrdering(): void
    {
        ...
        $url = \sprintf(
            '/molshoop/season/krtek/quiz/%s/question/%s/edit',
            $quiz->id,
            $question->id,
        );
    ...
    public function testReorderQuestionsWithinQuiz(): void
    {
        ...
        $overviewUrl = \sprintf('/molshoop/season/krtek/quiz/%s/overview', $quiz->id);
        ...
        $reorderUrl = \sprintf('/molshoop/season/krtek/quiz/%s/questions/reorder', $quiz->id);
    ...
    public function testReorderIsDeniedForNonOwner(): void
    {
        ...
        $this->loginAs('krtek-admin@example.org');
        $crawler = $this->client->request(Request::METHOD_GET, \sprintf('/molshoop/season/krtek/quiz/%s/overview', $quiz->id));
        ...
        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/quiz/%s/questions/reorder', $quiz->id), [
            '_token' => $csrfToken,
