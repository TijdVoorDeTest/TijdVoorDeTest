...
use Tvdt\Controller\Molshoop\SettingsController;
...
final class SettingsControllerTest extends AbstractControllerWebTestCase
{
    ...
    {
        return $this->getCsrfTokenFromPage('/molshoop/settings', $formActionContains);
    }
    ...
    public function testSettingsPageLoadsAndNavContainsSettingsLink(): void
    {
        ...
        self::assertSelectorExists('nav a[href="/molshoop/settings"]');
    }
    ...
    {
        $this->client->request(Request::METHOD_GET, '/molshoop/settings');
        $form = $this->client->getCrawler()->filter('form[action*="/molshoop/settings/password"]')->form([
    ...
    {
        $this->client->request(Request::METHOD_GET, '/molshoop/settings');
        $form = $this->client->getCrawler()->filter('form[action*="/molshoop/settings/email"]')->form([
    ...
    {
        $token = $this->getCsrfTokenFromSettings('/molshoop/settings/resend-confirmation');

        $this->client->request(Request::METHOD_POST, '/molshoop/settings/resend-confirmation', [
            '_token' => $token,
        ]);
    ...
    public function testResendConfirmationEmailForVerifiedUserSendsNothing(): void
    {
        ...
        $crawler = $this->client->request(Request::METHOD_GET, '/molshoop/settings');
        self::assertResponseIsSuccessful();
        $this->assertCount(0, $crawler->filter('form[action*="/molshoop/settings/resend-confirmation"]'));
    ...
    {
        $token = $this->getCsrfTokenFromSettings('/molshoop/settings/delete');

        $this->client->request(Request::METHOD_POST, '/molshoop/settings/delete', [
            '_token' => $token,
    ...
    {
        $this->client->restart();
        $this->client->request(Request::METHOD_GET, '/molshoop/settings/download-data');
