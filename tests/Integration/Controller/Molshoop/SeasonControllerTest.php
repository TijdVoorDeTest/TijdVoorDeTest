...
final class SeasonControllerTest extends AbstractControllerWebTestCase
{
    ...
    {
        $oldCode = 'krtek';
        $token = $this->getCsrfTokenFromPage(\sprintf('/molshoop/season/%s/settings', $oldCode), '/regenerate-code');

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/%s/settings/regenerate-code', $oldCode), [
            '_token' => $token,
        ]);
        ...
        $this->assertMatchesRegularExpression('#^/molshoop/season/[a-z]{5}/settings$#', $location);
    }

    public function testRegenerateSeasonCodeIsDeniedForNonOwner(): void
    {
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/settings', '/regenerate-code');

        $this->loginAs('test@example.org');

        $this->client->request(Request::METHOD_POST, '/molshoop/season/krtek/settings/regenerate-code', [
            '_token' => $token,
        ]);
    ...
    public function testRenameCandidate(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/candidates', \sprintf('/candidate/%s/rename', $candidate->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/candidate/%s/rename', $candidate->id), [
            '_token' => $token,
            'name' => 'Tommy',
        ]);

        self::assertResponseRedirects('/molshoop/season/krtek/candidates');
        $this->entityManager->clear();
    ...
    public function testDeleteCandidate(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/candidates', \sprintf('/candidate/%s/delete', $candidate->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/candidate/%s/delete', $candidate->id), [
            '_token' => $token,
        ]);
    ...
    {
        $this->client->request(Request::METHOD_GET, '/molshoop/season/krtek/add-candidate');
    ...
    {
        $this->client->xmlHttpRequest(Request::METHOD_GET, '/molshoop/season/krtek/add-candidate', server: ['HTTP_TURBO-FRAME' => 'add-candidates-modal-frame']);
        $form = $this->client->getCrawler()->filter('form')->form([
    ...
    {
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/settings', '/add-owner');

        $this->client->request(Request::METHOD_POST, '/molshoop/season/krtek/settings/add-owner', [
            '_token' => $token,
    ...
    public function testRemoveOwnerRemovesOwnership(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/settings', \sprintf('/owner/%s/remove', $user2->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/settings/owner/%s/remove', $user2->id), [
            '_token' => $token,
        ]);
    ...
    public function testRemoveOwnerOfNonMemberDoesNothing(): void
    {
        ...
        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/settings/owner/%s/remove', $nonMember->id), [
            '_token' => $token,
        ]);
    ...
    public function testRemoveOwnerIsBlockedWhenItWouldLeaveNoOwners(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/doomd/settings', \sprintf('/owner/%s/remove', $soleOwner->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/doomd/settings/owner/%s/remove', $soleOwner->id), [
            '_token' => $token,
        ]);

        self::assertResponseRedirects('/molshoop/season/doomd/settings');
        $this->entityManager->clear();
    ...
    public function testLeaveSeasonRemovesOwnershipAndRedirectsToIndex(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/settings', \sprintf('/owner/%s/remove', $krtekAdmin->id));

        $this->client->request(Request::METHOD_POST, \sprintf('/molshoop/season/krtek/settings/owner/%s/remove', $krtekAdmin->id), [
            '_token' => $token,
        ]);
    ...
    public function testDeleteSeasonWithCorrectConfirmationSoftDeletesSeason(): void
    {
        ...
        $token = $this->getCsrfTokenFromPage('/molshoop/season/krtek/settings', '/settings/delete');

        $this->client->request(Request::METHOD_POST, '/molshoop/season/krtek/settings/delete', [
            '_token' => $token,
